...
if (shop) {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            <div class="imgbox"><img src="${picture[0].src}" alt=""></div><span class="p-title">${el.title}</span><span class="p-price">${parseFloat(el.price)}</span>
            <div class="p-sum">
                <i class="numD">-</i><input type="text" value="${arr[0].num}"><i class="numU">+</i>
            </div><span class="p-total">${arr[0].num*parseFloat(el.price)}</span>
            <i class="p-del">X</i>
        </li>
            `;
